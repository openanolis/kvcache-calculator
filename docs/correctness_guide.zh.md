...
| `heuristic_report.zh.md` / `heuristic_report.en.md` | 无 trace heuristic 的假设、参数和边界说明 |
| `calibration.json` / `calibration_trials.csv` | trace 回标结果；只说明参数贴合程度，不说明“证明正确” |
| `recommended_heuristic_config.json` | trace 结构建议模板；只说明“更贴近样本的结构假设” |
...
如果还做了 trace 回标，那么又多出第四件事：

4. 在结构参数固定的前提下，用一小段真实 trace 回标 `zipf_s` 和 `lru_like`，并显式输出误差而不是只给参数。
...
- trace 回标只能说明“更贴近这段样本”，不能说明“以后所有 workload 都会这样”。
- trace 结构建议器只能说明“这组 `shared/private` 假设更贴近样本”，不能把推荐配置说成 workload 真相。
