...
> 这份文档把业务分析重写成一个四层框架。它的目的不是替代当前实现，而是给当前实现一个正确的位置，并给后续演进划清边界。

---

## 背景：为什么必须重写成四层

业务真正想回答的问题其实不是一个，而是一串因果链：
...
- 当前仓库的主能力属于 **Layer 1**
- 业务文档 `2.1 LRU` 更像 **Layer 2**
- 业务文档 `2.2 TPS 修正模型` 属于 **Layer 3**
- 业务文档 `2.3 Agent / Zipf` 属于 **Layer 4**
...
因为当前项目的业务语义是 strict-prefix prefill reuse：
...
### 业务文档 2.3 可以保留，但必须降级为 heuristic
...
| 路线 | 价值 | 风险 | 建议 |
|------|------|------|------|
| **先做 Policy/Economics** | ✅ 业务立刻可用 | ❌ 容易口径漂移 | 不建议直接跳 |
...
| 结论 | 判断 |
|------|------|
| 当前实现和业务文档是否完全同路？ | ❌ 不是完全同层 |
| 当前实现是否错了？ | ❌ 没错，它是在做更底层的 Oracle |
| 业务文档是否需要重写？ | ✅ 需要，必须按四层切开 |
