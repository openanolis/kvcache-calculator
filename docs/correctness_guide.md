...
- 它不参与 `TPS Gain`、`估算总 TPS`、`同负载估算卡数`、`同负载估算机器数` 的主计算。

规划结果仍然统一基于 `exact strict-prefix`，因为这才是容量语义下的最优值。
...
- `planning_summary.csv` 里的规划列全部按这套公式计算。
...
5. 最后再看 `planning_summary.csv` 里的 `TPS Gain / 估算总 TPS / 同负载估算卡数 / 同负载估算机器数`。
...
| `planning_summary.csv` | 这些命中率能换成多少 TPS、多少卡、多少机器 |
