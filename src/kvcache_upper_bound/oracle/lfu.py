...
class LFUSimulationResult:
    request_metrics: list[CapacityRequestMetric]
    summary: CapacitySummary


def analyze_lfu_baseline(
    ...
    event_hits = _run_lfu(access_trace.access_events, resident_block_capacity)
    ...
    return LFUSimulationResult(request_metrics=request_metrics, summary=summary)


def _run_lfu(access_events: Iterable[int], resident_block_capacity: int) -> bytearray:
    """LFU eviction: evict block with lowest frequency; ties broken by earliest insertion."""
    ...
    for index, node_id in enumerate(access_list):
        ...
        if len(resident) >= resident_block_capacity:
            # Evict block with lowest frequency; ties broken by earliest insertion
            victim = min(resident, key=lambda nid: (freq[nid], insertion_order[nid]))
            resident.remove(victim)
            del freq[victim]
            del insertion_order[victim]
