...
class MultiAgentHeuristicTest(unittest.TestCase):
    ...
    def test_cli_writes_multi_agent_outputs_and_metadata(self) -> None:
        ...
        self.assertIn("HBM Strict-Prefix Hit Rate (Estimate)", summary_rows[0])
        self.assertIn("HBM LRU-like Hit Rate (Estimate)", summary_rows[0])
        self.assertIn("Strict-Prefix Hit Rate (Estimate)", tier_rows[0])
        self.assertIn("LRU-like Hit Rate (Estimate)", tier_rows[0])
